...
const LessonQuizPane = ({ quizMeta, courseId, moduleId, lessonId, onQuizCompleted }) => {
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      ...
      if (!quizRes.success) {
        throw new Error(quizRes.error || 'Failed to load quiz details');
      }

      setQuiz(quizRes.data);
      const sortedAttempts = sortAttemptsDesc(attemptRes.success ? attemptRes.data || [] : []);
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      if (!res.success) {
        throw new Error(res.error || 'Unable to start quiz');
      }
                      ...
                      {question.options.map((option) => {
                        ...
                        return (
                          <label
                            key={fieldId}
                            htmlFor={fieldId}
                            className={`flex cursor-pointer items-center gap-3 rounded border p-2 text-sm ${isChecked ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                              }`}
                          >
                            <input
                              id={fieldId}
